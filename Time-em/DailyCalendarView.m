...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor blackColor]: [UIColor colorWithHex:0x0081c1];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor whiteColor]:[UIColor blackColor];
    }else{
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor blackColor]: [UIColor clearColor];
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor darkGrayColor]:[UIColor blackColor];
}
